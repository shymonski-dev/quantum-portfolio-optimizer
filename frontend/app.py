...
def classify_error(error_msg: str) -> dict:
    ...
    for key, guidance in ERROR_GUIDANCE.items():
        if key.lower() in error_str:
            return {"message": guidance, "technical": str(error_msg), "type": key}
    return {"message": str(error_msg), "technical": str(error_msg), "type": "unknown"}
...
def optimize():
    ...
    try:
        ...
        tickers = [
            t.strip().upper()
            for t in data.get("tickers", "AAPL,GOOG,MSFT,AMZN").split(",")
        ]
        start_date = data.get("start_date", "2023-01-01")
        end_date = data.get("end_date", "2024-01-01")
        risk_factor = float(data.get("risk_factor", 0.5))
        ansatz_type = (
            data.get("ansatz", "real_amplitudes")
            .lower()
            .replace("realamplitudes", "real_amplitudes")
            .replace("efficientsu2", "efficient_su2")
        )
        ...
        if esg_scores_raw and str(esg_scores_raw).strip():
            try:
                parsed = [
                    float(v.strip())
                    for v in str(esg_scores_raw).split(",")
                    if v.strip()
                ]
                if parsed:
                    esg_scores = parsed
        ...
        logger.info(
            f"Starting optimization: {tickers}, {start_date} to {end_date} via {data_provider}"
        )
        logger.info(
            f"Settings: algorithm={algorithm}, risk={risk_factor}, backend={backend_type}"
        )
        if algorithm == "vqe":
        ...
        stock_data = fetch_stock_data(
            tickers, start_date, end_date, provider=data_provider
        )

        if stock_data.empty:
            return jsonify(
                {"error": "Failed to fetch stock data. Check tickers and dates."}
            ), 400
        ...
        if auto_cluster:
            try:
                ...
                sectors = get_optimal_cut_points(
                    returns_df, max_cluster_size=max(2, num_assets // 2)
                )
        ...
        logger.info(
            f"QUBO built: {num_assets} assets x {resolution_qubits} resolution = {total_qubits} qubits"
        )
        ...
        if backend_type == "ibm_quantum":
            ...
            if ibm_channel == "ibm_cloud":
                if not ibm_crn:
                    return jsonify(
                        {
                            "error": "Cloud Resource Name (CRN) is required for IBM Cloud access"
                        }
                    ), 400
            ...
            backend_config = {
                ...
                "error_mitigation": {
                    ...
                    "dynamical_decoupling": parse_bool(
                        data.get("dd_enabled", True), True
                    ),
                    "dd_sequence": data.get("dd_sequence", "XpXm") or "XpXm",
                    "twirling_enabled": parse_bool(
                        data.get("twirling_enabled", True), True
                    ),
                    "zne_enabled": parse_bool(data.get("zne_enabled", False), False),
                    "zne_noise_factors": zne_noise_factors,
                    "zne_extrapolator": data.get("zne_extrapolator", "exponential")
                    or "exponential",
                },
            }
            ...
            if isinstance(raw_noise, str) and raw_noise.strip():
                try:
                    local_zne_factors = [
                        float(v.strip()) for v in raw_noise.split(",") if v.strip()
                    ]
                except ValueError:
        ...
        if algorithm == "vqe":
            ansatz_for_solver = get_ansatz(
                ansatz_type, num_qubits=total_qubits, reps=reps
            )
        ...
        if use_warm_start:
            try:
                ...
                if algorithm == "qaoa":
                    ...
                    logger.info(
                        f"Warm start (QAOA): estimated {warm_start_result.estimated_improvement:.1f}x improvement"
                    )
                else:  # VQE
                    ...
                    initial_point = warm_start_result.initial_parameters.tolist()
                    logger.info(
                        f"Warm start (VQE): estimated {warm_start_result.estimated_improvement:.1f}x improvement"
                    )
            except Exception as e:
        ...
        elif backend_type == "ibm_quantum" and backend_config.get(
            "error_mitigation", {}
        ).get("zne_enabled"):
        ...
        if algorithm == "qaoa":
            ...
            if mixer_type == "xy":
                xy_num_assets = (
                    num_assets_select if num_assets_select else max(1, num_assets // 2)
                )
        ...
        expected_return = (
            sum(w * r for w, r in zip(weights, mean_returns)) * 252
        )  # Annualized
        portfolio_variance = (
            sum(
        ...
        classical_result = markowitz_baseline(
            ...
            budget=1.0,
            risk_aversion=risk_aversion
            / 10000,  # Scale down to match classical range (0-1)
        )
        ...
        classical_sharpe = (
            classical_return / classical_risk if classical_risk > 0 else 0
        )
        ...
        response = {
            ...
            "warm_start": {
                ...
                "estimated_improvement": round(
                    warm_start_result.estimated_improvement, 1
                )
            ...
            "classical_baseline": {
                ...
                "allocations": {
                    t: round(w * 100, 1)
                    for t, w in zip(tickers, classical_result.allocations)
                },
                "success": classical_result.success,
            },
            "mip_baseline": {
                "expected_return": round(mip_return * 100, 2)
                if mip_result and mip_result.success
                else None,
                "portfolio_risk": round(mip_risk * 100, 2)
                if mip_result and mip_result.success
                else None,
                "sharpe_ratio": round(mip_sharpe, 2)
                if mip_result and mip_result.success
                else None,
                "allocations": (
                    {
                        t: round(w * 100, 1)
                        for t, w in zip(tickers, mip_result.allocations)
                    }
                    if mip_result and mip_result.success
                    else None
                ),
                ...
                if mip_result
                else "MIP baseline not computed",
            },
            "qaoa_settings": {
            ...
            "esg": {
                ...
                "weight": esg_weight,
                "scores": {t: round(s, 2) for t, s in zip(tickers, esg_scores)}
                if esg_scores
                else None,
            },
            "convergence": {
                "history": result.history[:100]
                if len(result.history) > 100
                else result.history,
                "best_history": result.best_history[:100]
                if len(result.best_history) > 100
                else result.best_history,
            ...
            "quality_score": {
                ...
                "grade": quality.grade,
                "components": {
                    k: round(v, 1) for k, v in quality.component_scores.items()
                },
        ...
        if result.measurement_counts:
            ...
            sorted_counts = sorted(
                result.measurement_counts.items(), key=lambda x: x[1], reverse=True
            )[:5]
    ...
    backends = [
        {
            ...
            "description": "Fast, runs on your computer",
        },
    ]
...
if __name__ == "__main__":
    ...
    print("=" * 60 + "\n")
    debug_mode = os.environ.get("FLASK_DEBUG", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
