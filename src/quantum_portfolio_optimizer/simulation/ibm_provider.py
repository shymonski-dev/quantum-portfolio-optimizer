...
class ErrorMitigationConfig:
    """Configuration for IBM Quantum error mitigation options (Resilience V2).

    Attributes:
        zne_enabled: Enable Zero-Noise Extrapolation
        pec_enabled: Enable Probabilistic Error Cancellation (advanced PEC/V2)
...
class IBMQuantumConfig:
    ...
    Attributes:
        ...
        channel: Channel type ('ibm_quantum_platform' for open, 'ibm_cloud' for premium)
        ...
        use_ai_transpiler: Enable 2026-era AI-driven circuit optimization
...
def _build_estimator_options(config: IBMQuantumConfig) -> "EstimatorOptions":
    """Build EstimatorOptions with 2026-era error mitigation features."""
    ...
    # Dynamical decoupling (cloud-side execution)
    ...
    # ZNE (Zero-Noise Extrapolation) - Resilience V2 (2025 standard)
    ...
    # PEC (Probabilistic Error Cancellation) - Resilience Level 3 (2026 Standard)
    if em.pec_enabled and em.resilience_level >= 3:
...
def _build_sampler_options(config: IBMQuantumConfig) -> "SamplerOptions":
    """Build SamplerOptions with 2026-era execution features."""
...
def get_ibm_quantum_backend(config: Dict[str, Any]) -> Tuple[Any, Any]:
    ...
    Args:
        config: Backend configuration dictionary with keys:
            ...
            - channel: 'ibm_quantum_platform' or 'ibm_cloud' (default: 'ibm_quantum_platform')
    ...
    use_ai_transpiler = config.get("use_ai_transpiler", True)
    ai_transpilation_level = config.get("ai_transpilation_level", 2)

    # Parse error mitigation config
    em_config = config.get("error_mitigation", {})
    error_mitigation = ErrorMitigationConfig(
        zne_enabled=em_config.get("zne_enabled", True),
        pec_enabled=em_config.get("pec_enabled", False),
        ...
        resilience_level=em_config.get("resilience_level", 2),
    )
    ...
    ibm_config = IBMQuantumConfig(
        ...
        use_ai_transpiler=use_ai_transpiler,
        ai_transpilation_level=ai_transpilation_level,
    ...
    if use_session:
        ...
        # Attach backend object for ISA transpilation access
...
def list_available_backends(channel: str = "ibm_quantum_platform") -> list:
